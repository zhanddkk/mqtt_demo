...
bool EventLogDb::process(EventLogDataToNmcValue *val) {
	bool ret = false;
#if __WORDSIZE == 64
	...
	if (ts.tv_sec >= (m_update_ts.tv_sec + 1)) {
		...
		printf("[%ld.%09ld] W: %10d -> %4d drop = %4d\n", ts.tv_sec,
			ts.tv_nsec, next_id, m_id - m_counts, drop_counts);
...
static void _message_callback(struct mosquitto *mosq, void *userdata,
	...
	if (message->payloadlen) {
		...
		int ret = ring_buffer_write(&user_data.rb, message->payload,
			message->payloadlen);
...
static void _connect_callback(struct mosquitto *mosq, void *userdata,
	int result) {
	if (!result) {
		mosquitto_subscribe(mosq, NULL, "UPSSystem/General/EventLogDataToNMC",
			0);
	} else {
	...
	while (true) {
		if (sem_wait(&_rb_process_sem) == 0) {
			...
			if (dln > 0) {
				...
				if (decode_payload((const uint8_t *) backup_data, dln,
					payload)) {
					EventLogDataToNmcValue *val =
						dynamic_cast<EventLogDataToNmcValue *>(payload.p_value);
					if (user_data.write_db) {
						db.process(val);
					} else {
						static struct timespec _update_ts = { 0, 0 };
						static uint32_t _p_counts = 0, _p_counts_snap = 0;
						struct timespec ts;
						_p_counts++;
						clock_gettime(CLOCK_MONOTONIC, &ts);
						if (ts.tv_sec >= _update_ts.tv_sec + 1) {
							uint32_t cnt =
								_p_counts >= _p_counts_snap ?
									_p_counts - _p_counts_snap :
									0xffffffffu - _p_counts_snap + _p_counts;
							_update_ts = ts;
							printf("[%ld.%09ld] P: %10d -> %4d drop = %4d\n",
								ts.tv_sec, ts.tv_nsec, _p_counts, cnt,
								drop_counts);
							_p_counts_snap = _p_counts;
						}
					}
				} else {
		...
		if (mosq) {
			...
			if (mosquitto_username_pw_set(mosq,
				...
				if (mosquitto_connect(mosq, host, port, keepalive)
					...
					if (ring_buffer_init(&user_data.rb, MAX_MSG_SIZE,
						MAX_BUFFER_SIZE) == 0) {
